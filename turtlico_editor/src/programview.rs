...
impl ProgramViewState {
    ...
    pub fn insert(&mut self, relpos: Pos2, data: EditorDragData) {
        {
            ...
            let col_max = if row < project.program.len() {
                project.program[row].len() - 1
            } else {
                0
            };
            ...
            project.insert(
                data.commands,
                usize::min(col, col_max),
                usize::min(row, row_max),
            );
        }
    }
    ...
    pub fn get_cmd_at_pointer(
        ...
        ui: &mut egui::Ui,
        rect: Rect,
    ) -> Option<(crate::project::Command, CommandRange)> {
        let mut result = None;
        ui.input(|i| {
            if let Some(ptrpos) = i.pointer.interact_pos() {
                ...
                if row < project.program.len() && col < project.program[row].len() {
                    result = Some((
                        project.program[row][col].clone(),
                        CommandRange::single_icon(col, row),
                    ));
                }
            }
        });
                ...
                egui::ScrollArea::both().show(ui, |ui| {
                    let (rect, response) = ui.allocate_at_least(
                        Vec2::new(
                            f32::max(
                                ui.available_width(),
                                state.size.width() + CMD_ICON_SIZE_VEC.x * 3.0,
                            ),
                            f32::max(
                                ui.available_height(),
                                state.size.height() + CMD_ICON_SIZE_VEC.y * 3.0,
                            ),
                        ),
                    ...
                    if ui.is_enabled() {
                        ...
                        if response.secondary_clicked() {
                            if let Some((cmd, range)) = state.get_cmd_at_pointer(ui, rect) {
                                state.edited_cmd =
                                    Some((range.start.0, range.start.1, cmd.clone()));
                            }
                        }
                    }
                    ...
                    if ui.is_rect_visible(rect) {
                        ...
                        project.renderer.as_ref().unwrap().render_block(
                            &project.program,
                            &project,
                            &painter,
                            rect.min,
