...
pub fn parse(source: &str) -> Result<Spanned<Expression>, Vec<Spanned<Error>>> {
    ...
    let lexer_out = lexer
        .spanned()
        .collect::<Vec<(Result<Token, ()>, Range<usize>)>>();
    let mut tokens = vec![];
    let mut errors = vec![];
    ...
    create_parser()
        .parse(Stream::from_iter(srclen..srclen + 1, tokens.into_iter()))
        .map_err(|errors| {
            errors
                .into_iter()
                .map(|err| {
    ...
    let expression = recursive(|expr| {
        let block = (expr.clone().repeated())
            .delimited_by(
                padding
                    .clone()
                    .then(just(Token::LeftCurly))
                    .then(padding.clone()),
                just(Token::RightCurly),
            )
            .map(Expression::Block)
            .map_with_span(Spanned::new);
        let literal = filter_map(|span, token: Token| match token {
            ...
            Token::Float(x) => x
                .parse()
                .map_err(|err: ParseFloatError| Simple::custom(span, err.to_string()))
                .map(|value| Expression::Float(value)),
            _ => Err(Simple::custom(span, "Not a literal")),
        })
        .map_with_span(Spanned::new);
        ...
        let objdef = expr
            ...
            .delimited_by(just(Token::LeftCurly).then(padding.clone()), just(Token::RightCurly))
            .map(|items| {
                let mut object = vec![];
                for item in items {
        ...
        let var = (var_name
            ...
            .map(|(parent, key)| {
                ...
                Expression::Variable {
                    parent: Some(Box::new(p_ex)),
                    name: key.item,
                }
            }))
        .or(var_name.map(|v| Expression::Variable {
            parent: None,
            name: v.item,
        }))
        .map_with_span(Spanned::new);
        ...
        .or(func_name.map(|v| Expression::Variable {
            parent: None,
            name: v.item,
        }))
        .map_with_span(Spanned::new);
        ...
        let call = func
            ...
            .map(|(ident, args)| Expression::Call {
                expr: Box::new(ident),
                args: args,
            })
            .map_with_span(Spanned::new);
        ...
        let shortcall = func
            .clone()
            .then(
                shortcall_param.clone().repeated(), /*.separated_by(just(Token::Comma))*/
            )
            .map(|(ident, args)| Expression::Call {
                expr: Box::new(ident),
                args: args,
            })
            .or(func.clone().map(|ident| Expression::Call {
                expr: Box::new(ident),
                args: vec![],
            }))
            .map_with_span(Spanned::new);
        ...
        let assignment = var
            ...
            .then(expr.clone())
            .map(|(expr, value)| Expression::Assignment {
                expr: Box::new(expr),
                value: Box::new(value),
            })
            .map_with_span(Spanned::new);
        ...
        let condition = just(Token::If)
            .then(expr.clone())
            .then(expr.clone())
            .map(|((_token, cond), body)| Expression::If {
                cond: Box::new(cond),
                body: Box::new(body),
            })
            .map_with_span(Spanned::new);
        ...
        let ret = just(Token::Return)
            .then(expr.clone())
            .map(|(_token, expr)| Expression::Return {
                value: Box::new(expr),
            })
            .or(just(Token::Return).map(|_token| Expression::Return {
                value: Box::new(Spanned::new(Expression::None, 0..0)),
            }))
            .map_with_span(Spanned::new);
        ...
        let loop_finite = just(Token::Loop)
            .then(expr.clone())
            .then(block.clone())
            .map(|((_token, iters), body)| Expression::LoopFinite {
                iters: Box::new(iters),
                body: Box::new(body),
            })
            .map_with_span(Spanned::new);
        let loop_infinite = just(Token::Loop)
            .then(block.clone())
            .map(|(_token, body)| Expression::LoopInfinite {
                body: Box::new(body),
            })
            .map_with_span(Spanned::new);
        let loop_for = just(Token::For)
            .then(var_name.clone())
            .then(expr.clone())
            .then(expr.clone())
            .then(expr.clone())
        ...
        let loop_while = just(Token::While)
            .then(expr.clone())
            .then(expr.clone())
            .map(|((_token, cond), body)| Expression::While {
                cond: Box::new(cond),
                body: Box::new(body),
            })
            .map_with_span(Spanned::new);
        ...
        let fndef = just(Token::FnDef)
            ...
            .then(
                (select! {Token::Variable(x) => x})
                    .separated_by(just(Token::Comma))
                    .delimited_by(just(Token::LeftParent), just(Token::RightParent)),
            )
            .then(expr.clone())
            .map(|(((_token, name), args), body)| Expression::FnDef {
                name: name,
                args: args,
                body: Box::new(body),
            })
        ...
        let unary = just(Token::Minus)
            ...
            .foldr(|op, rhs| {
                ...
                Spanned::new(
                    Expression::Negation(Box::new(rhs)),
                    Range {
                        start: op.span.start,
                        end: end,
                    },
        ...
        let product = unary
            ...
            .foldl(|lhs, (op, rhs)| {
                ...
                let end = rhs.span.end;
                Spanned::new(
                    (op.item)(Box::new(lhs), Box::new(rhs)),
                    Range {
                        start: start,
                        end: end,
                    },
        ...
        let sum = product
            ...
            .then(
                just(Token::Plus)
                    ...
                    .map_with_span(Spanned::new)
                    .or(just(Token::Minus)
                        .to(Expression::Subtraction as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .then(product)
                    .repeated(),
            )
        ...
        let eq = sum
            ...
            .then(
                just(Token::Eq)
                    ...
                    .map_with_span(Spanned::new)
                    .or(just(Token::Neq)
                        .to(Expression::Neq as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .or(just(Token::Lt)
                        .to(Expression::Lt as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .or(just(Token::Gt)
                        .to(Expression::Gt as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .or(just(Token::Lte)
                        .to(Expression::Lte as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .or(just(Token::Gte)
                        .to(Expression::Gte as fn(_, _) -> _)
                        .map_with_span(Spanned::new))
                    .then(sum)
                    .repeated(),
            )
