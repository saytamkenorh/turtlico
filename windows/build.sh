...
BUILD_DEPS="desktop-file-utils gcc meson pkg-config"
RUNTIME_DEPS=""
...
pacman -Su --noconfirm
pacman -S expac tar --needed --noconfirm

BUILD_DEPS=$(echo $pkglist | sed "s/[^ ]* */$MINGW_PREFIX-&/g")
RUNTIME_DEPS=$(echo $pkglist | sed "s/[^ ]* */$MINGW_PREFIX-&/g")

# Find deps using expac
build_deps_all=""
runtime_deps_all=""

install_list="$build_deps_all $runtime_deps_all"
pacman -S $install_list --needed --noconfirm
...
rm "$bindir/gdk-pixbuf-query-loaders"*
...
"./turtlico.exe" --compile="/dev/null" "../doc/examples/turtle-star.tcp"  || exit 1
