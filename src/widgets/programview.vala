...
namespace Turtlico {
    ...
    public class ProgramView : Gtk.DrawingArea {
        ...
        void on_drag_data_received (Gdk.DragContext context,
        ...
        public override bool draw (Cairo.Context cr) {
            ...
            for (int line = 0; line < buffer.program.size; line++) {
                ...
                for (int command = 0; command < buffer.program[line].size; command++) {
                    ...
                    // Selection
                    bool on_selection_point =
                        (line == buffer.selection_start.y && command == buffer.selection_start.x) ||
            ...
            if (!(c.definition.data_draw && c.definition.data_only) || c.data == "") {
                if (c.pixbuf != null) {
                    ...
                    Cairo.Surface pb = Gdk.cairo_surface_create_from_pixbuf (
                        c.pixbuf, get_scale_factor (), get_window ());
            ...
            if (event.button == Gdk.BUTTON_PRIMARY) {
                if (buffer.selection_phase == SelectionPhase.SELECT_END) {
                    ...
                    if (buffer.selection_end.y < buffer.selection_start.y)
                        do_swap = true;
                    if (buffer.selection_end.y == buffer.selection_start.y &&
                        buffer.selection_start.x > buffer.selection_end.x
                    )
                        do_swap = true;
                    if (do_swap) {
